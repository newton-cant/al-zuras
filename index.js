...
const antifake = JSON.parse(fs.readFileSync('./data/antifake.json'));
  ...
  only: {
    ...
    userB: `🍒*${pushname2}*, 𝚅𝙾𝙲𝙴 𝙽𝙰𝙾 𝙴𝚂𝚃𝙰 𝚁𝙴𝙶𝙸𝚂𝚃𝚁𝙰𝙳𝙾 𝙽𝙾 𝙱𝙰𝙽𝙲𝙾 𝙳𝙴 𝙳𝙰𝙳𝙾𝚂, 𝙳𝙸𝙶𝙸𝚃𝙴 \n*${prefix}daftar*🍒`,
      ...
      const totalchat = await Pin.chats.all()
		    ...
		    const ownerNumber = ["19388881408@s.whatsapp.net"] // replace this with your number
...
┃❏│⊱❥ GRUPO: *${groupName}*
┃❏│⊱❥ N USUÁRIOS: *${user.length} User*
┃❏│⊱❥ N CHATS: *${totalchat.length} Chat*
┃❏│⊱❥ CRIAD : *SUPRA*
╰══════════════════⊷❍
┃─────〘 *LHANNA-BOT* 〙────
  ...
  break
  		 ...
  		 case 'casal':
				...
				case 'gados':
					...
					const supr = neww[Math.floor(Math.random() * neww.length)]
...
case 'sticker1':
   if (!isUser) return reply(mess.only.userB)
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  tels = body.slice(6)
	  anu = await imgbb("b2f7f9943262ff49d9d95c4e18b7d4da", owgi)
	  hehe = await getBuffer(`https://lolhuman.herokuapp.com/api/convert/towebp?apikey=1235bf8d15eda9105ce76f50&img=${anu.display_url}`)
...
case 'sticker2':
   if (!isUser) return reply(mess.only.userB)
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  tels = body.slice(6)
	  anu = await imgbb("b2f7f9943262ff49d9d95c4e18b7d4da", owgi)
	  hehe = await getBuffer(`https://lolhuman.herokuapp.com/api/convert/towebp?apikey=6b25e69d0ba3dc9447010464&img=${anu.display_url}`)
	 Pin.sendMessage(from, hehe, sticker, {quoted:mek})
	} else {
	  reply('Jangan tambah kan apapun pada command')
	}
...
case 'casalnv':
  ...
  const ll = groupMembers
   const gg = dd[Math.floor(Math.random() * dd.length)]
	...
	teks = `*Novo casal do grupo*:\n@${gg.jid.split('@')[0]} ❤️\n @${rr.jid.split('@')[0]} ❤️ `	
      data = fs.readFileSync('./apis/roman.js');
      jsonData = JSON.parse(data);
      randIndex = Math.floor(Math.random() * jsonData.length);
      randKey = jsonData[randIndex];
      buffer = await getBuffer(randKey.result)
      Pin.sendMessage(from, buffer, video, {mimetype: 'video/mp4',quoted: mek, caption: teks})
      break
      
case 'blackpinkv':
      if (!isGroupAdmins) return reply(mess.only.admin)
      data = fs.readFileSync('./apis/blackpink.js');
      jsonData = JSON.parse(data);
      randIndex = Math.floor(Math.random() * jsonData.length);
      randKey = jsonData[randIndex];
      buffer = await getBuffer(randKey.result)
...
case 'gado':
   ...
   case 'namorar':
    Pin.updatePresence(from, Presence.composing) 
     random = `${Math.floor(Math.random() * 100)}`
     hasil = `Veja a chance de você namorar esse ano:\n\na chance de você namorar esse ano é: *${random}%* `
     reply(hasil)
     break
...
Pin.sendMessage(from, dador, sticker, {quoted: mek})
break
...
cararo = ["cara", "coroa"]
fit = cararo[Math.floor(Math.random() * cararo.length)]
aa = fit
reply(`você conseguiu: ${fit}`)
cararoa = fs.readFileSync('./database/cara/'+fit+'.webp')
Pin.sendMessage(from, cararoa, sticker, {quoted: mek})
break
...
const sn = ['sim', 'não']
gosto = body.slice(3)
if (args.length < 1) return Pin.sendMessage(from, `Você deve fazer uma pergunta...\nExemplo: ${prefix}sn O supra é um baiano preguiçoso?`, text, {quoted: mek})
...
case 'slot':
const somtoy = [Math.floor(Math.random())]	
...
Pin.sendMessage(from, slott, text, {quoted: mek})
break


case "ppt":
if (args.length < 1) return reply('digite')
...
if (vit == "derrota" ) {
var tes = "A vitória é da Lhanna-BOT"
}
if (vit == "empate" ) {
var tes = "O jogo terminou em empate"
}
reply(`Lhanna-BOT jogou: ${pptb}\nO jogador jogou: ${args}\n\n${tes}`)
...
` INFORMAÇÕES DO DDD
  ...
  ‣ Cidades: 
    ${hehe.cities}\n`
Pin.sendMessage(from, ccg, text, {quoted:mek})
break
...
Pin.sendMessage(from, anu, image, {quoted: mek})
break
...
case 'conselho':
 ...
 if (!isUser) return reply(mess.only.userB)
  try {
   ...
   randKey = jsonData[randIndex];
     ...
     case 'punheteiro':
	memein = await kagApi.memeindo()
	buffer = await getBuffer(`http://cdn30.us1.fansshare.com/image/miakhalifa/mia-khalifa-in-bikini-in-the-shower-muslim-673235137.jpg`)
    Pin.updatePresence(from, Presence.composing) 
     random = `${Math.floor(Math.random() * 100)}`
     hasil = `*NUNCA BATEU UMA?* 🙂\n\nSeu nível de punheteiro amg:\n\nVocê é: *${random}%*PUNHETEIRO!!\n*JÁ DESCONFIAVA* 😂\n\n*Lhanna-BOT*`
     Pin.sendMessage(from, buffer, image, {quoted: mek, caption: hasil})
     break
 ...
 play = body.slice(6)
 reply('Lhanna está procurando sua música...⌛')
 anu = await fetchJson(`https://api.zeks.xyz/api/ytplaymp3?apikey=S38aL2CO2Ez4wZjJWxD2vaJKKrC&q=${play}`)
 buffer = await getBuffer(anu.result.thumbnail) 
 lagu = await getBuffer(anu.result.url_audio)
  Pin.sendMessage(from, buffer, image, {quoted: mek, caption:'LHANNA ESTÁ BAIXANDO SUA MÚSICA🎶🎶'})
...
case 'vid':
 ...
 case 'vid1':
   ...
   case 'vid2':
   play = body.slice(5)
   anu = await fetchJson(`https://api.zeks.xyz/api/ytplaymp4?apikey=S38aL2CO2Ez4wZjJWxD2vaJKKrC&q=${play}`)   
   buffer = await getBuffer(anu.result.thumbnail)
 ...
 case 'instvid':
   ...
   break
                  ...
                  if (args.length < 1) return reply('MASUKKAN LINK/URL FB') 
                    ...
                    Pin.sendMessage(from, anu2, text, {quoted: mek})
                    anu3 = await getBuffer(anu.result.video)
                    Pin.sendMessage(from, anu3, video, {mimetype: 'video/mp4', quoted: mek})
                    break 
...
.on('end', function () {
console.log('Finish')
exec(`webpmux -set exif ${addMetadata('Lhanna-BOT', 'supra')} ${ran} -o ${ran}`, async (error) => {
...
exec(`webpmux -set exif ${addMetadata('Lhanna-BOT', 'supra')} ${ranw} -o ${ranw}`, async (error) => {
if (error) return reply(ptbr.stick())
Pin.sendMessage(from, fs.readFileSync(ranw), sticker, {quoted: mek})
fs.unlinkSync(ranw)
})
...
case 'antiligar':
          Pin.on("CB:action,,call", async json => {
    ...
    await Pin.blockUser(callerId, "add"); // Block user
    //}
  });
             break
...
case 'antisticker':
            ...
            case 'antilink':
					...
					if (Number(args[0]) === 1) {
						...
						antifake.push(from)
						fs.writeFileSync('./data/antifake.json', JSON.stringify(antifake))
